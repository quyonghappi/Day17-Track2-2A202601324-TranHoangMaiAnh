...
def write_batch(con: duckdb.DuckDBPyConnection, batch: list[dict]) -> None:
    ...
    Upsert theo event_id làm phép ghi idempotent khi consumer phát lại batch
    sau sự cố. DO UPDATE cũng giữ payload mới nhất nếu message replay có nội
    dung đã đổi, thay vì âm thầm bỏ qua nó như DO NOTHING.
    """
    con.executemany(
        f"""
...
def consume(
    ...
    with LogConsumer(topic, offset_file) as consumer:
        ...
        while True:
            ...
            consumer.commit()                 # ghi nhận offset sau khi ghi
            # ─────────────────────────────────────────────────────────────
